...
class ConvNet(nn.Module):
    def __init__(self, ks_conv, str_conv, pad_conv, ks_pool, str_pool, pad_pool):
        ...
        self.seq  = 60 # sequence compressed to (encoder)
        ...
        self.fc1_encode = nn.Sequential(
            nn.Linear(100*60, 500),
            nn.ReLU(),
            nn.BatchNorm1d(500)) 
        
        ## DECODER ##   
        self.fc1_decode = nn.Sequential(
            nn.Linear(500, 100*60),
            nn.ReLU(),
            nn.BatchNorm1d(100*60))
...
#        out = self.fc2_decode(out)
        ...
        out = out.view(-1, 100,60) # re-shape array but not batches
