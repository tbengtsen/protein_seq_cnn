...
class RepresentationsDMS():
    ...
    def __init__(self,protein, model):
        ...
        self.wt = HandleFasta(protein)
        self.wt_seq =  self.wt.sequence # integer labels
        self.wt_seq_OH = self.wt.sequence_one_hot
    ...
    def split_train_test (self):
        ...
        train = self.dms_matrix[train_indx]
        test = self.dms_matrix[test_indx]
...
#     def repr_ddg(self, model, correct):
#         '''can't remember what it does......  '''
        


#         ## get WT representation (flatttend )
#         wt_repr = model.embed(self.wt_seq_OH)
        
#         lat_space = model.latent_size
...
#         ## add wt_repr to first 500 columns in ddgs_repr
#         ddgs_repr[:,:ls1] = wt_repr.cpu().numpy()

#         ## add WT labels to first columns between 1000:1500
#         ddgs_repr[:,ls3:ls4] = WT_seq

#         for indx,row in ddgs.iterrows():
#             mut_seq, idx_mut, mut_2_aa, wt_aa = self.get_mutation(idx, row)
            
#             # change from integer sequence to OH for CNN input
#             mut_seq_OH = HandleFasta().vocab_2_one_hot(mut_seq)
            
#             # get mutation representation (flattend)
#             mut_repr = model.embed(mut_seq_OH)

#             # add mut_repr to ddgs_repr
#             ddgs_repr[indx,ls1:ls2] = mut_repr.cpu().numpy()
...
def decoder_prediction(seq_input, model):
    ...
    with torch.no_grad():
        ...
        print('len input shape',len(seq_input.shape), 'shape',seq_input.shape)
        # if seq_input is single sequence not one hot encoded
        # convert to one hot 
