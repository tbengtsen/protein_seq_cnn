...
@author: nsde
"""
# avoid future warning when using older tensorflow version 
import warnings
...
# try to supress tensorflow stuff 
...
class Dataset(object):
    def __init__(self, batch_size=100, shuffle=True, max_length=500):
        ...
        self.test_files =  [self.folder + '/' + f for f in self.files if 'test' in f ]
#         self.test_files = [self.folder + '/' + f for f in self.files if 
#                            (f not in self.train_files and f not in self.val_files)] # old, error, reads all train+test+val
...
class FluorescenceDataset(Dataset):
    folder = '../../../data/pytorchtape/fluorescence'
    _train_N = 21446
    _val_N = 5362
    _test_N = 27217
#%%
class ProteinnetDataset(Dataset):
    folder = '../../../data/pytorchtape/proteinnet'
    _train_N = 25298
    _val_N = 223
    _test_N = 25562
...
class RemotehomologyDataset(Dataset):
    folder = '../../../data/pytorchtape/remote_homology'
    _train_N = 12311
    _val_N = 735
    _test_N = 16291
...
class SecondarystructureDataset(Dataset):
    folder = '../../../data/pytorchtape/secondary_structure'
    _train_N = 8677 
    _val_N = 2169
    _test_N = 11496
...
class StabilityDataset(Dataset):
    folder = '../../../data/pytorchtape/stability'
    _train_N = 53614
    _val_N = 2512
    _test_N = 12851 
