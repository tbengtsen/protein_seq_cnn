...
# convert PFAM_VOCAB to be used in Tone's trained models
PFAM_2_TONE = {}
for pfam_name, pfam_idx in PFAM_VOCAB.items():    
    if pfam_name in [ '<PAD>', '<MASK>','<CLS>','<SEP>' ]:
            PFAM_2_TONE [pfam_idx] = 0
    
    elif pfam_name in [ 'B',  'X', 'Z', 'J' ]:
            PFAM_2_TONE [pfam_idx] = np.nan # not in Tone_unirep chnls, thus, must be deleted after
    else:
        tone_idx = UNIREP_TONE_VOCAB[pfam_name]
        PFAM_2_TONE [pfam_idx] = tone_idx
